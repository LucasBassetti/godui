...
<Example
  ...
  code={`import { SplitFlapDisplay } from "@/components/godui/split-flap-display";
...
export function Clock() {
  ...
  return <SplitFlapDisplay value={time} align="center" size="sm" charset="0123456789" />;
}`}
>
  <SplitFlapClockDemo />
</Example>
...
Restrict the flap sequence to just the glyphs you need — a digits-only `charset`
keeps a clock's `9 → 0` roll a single clean flip (glyphs outside the charset,
like the `:`, simply snap into place instead of spinning):

```tsx
<SplitFlapDisplay value="12:45:07" size="sm" charset="0123456789" />
```
...
| `charset`  | `string`                          | A–Z 0–9 …    | Ordered glyphs the flaps rotate through. Leading space is the blank flap. |
