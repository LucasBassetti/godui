...
| `length`   | `number`                          | value length | Fixed flap count — finite integers clamp to `0–100`; non-finite or fractional values fall back to the value length, also capped at 100. |
...
| `maxFlaps` | `number`                          | `12`         | Cap on how many flips one column spins; finite non-negative values round down, while negative or non-finite values use `12`. |
