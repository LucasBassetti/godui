...
scroll while open. Use `side="right"` for a side panel. The portal defaults to the
document where the component is rendered; pass `container` to target a specific
document body or element.
...
| `container`    | `HTMLElement \| null` | Owning document body | Portal target for the drawer.           |
