...
The portal defaults to the document where the component is rendered; pass `container`
to target a specific document body or element.
...
| `container`      | `HTMLElement \| null`      | Owning document body          | Portal target; also determines listeners and locking. |
