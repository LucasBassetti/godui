...
`continuous` to loop it. Set the pace with `traceDuration` — the packets travel
at the same speed the borders trace, so the light never speeds up or slows down
at the card→line seam. (`flowDuration` sets packet speed only when you drive the
edges yourself, without `autoPlay`.)
...
| `flowDuration` | `number`          | `3`                | Seconds per packet travel (manual mode; `autoPlay` matches the border-trace speed). |
| `traceDuration`| `number`          | `0.8`              | Seconds for a node's border to trace on.             |
