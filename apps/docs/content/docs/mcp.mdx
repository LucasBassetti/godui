...
process serves the latest components — no MCP upgrade is needed when GodUI
ships something new. Each deployment publishes a versioned `manifest.json`;
the server verifies the SHA-256 digest of the index and component response
before returning it. Missing or mismatched entries fail closed. Point it at a
local registry for development by setting `GODUI_REGISTRY_URL`:

```bash
GODUI_REGISTRY_URL=http://localhost:3000/r npx @godui/mcp@0.1.0
```

The live endpoint is freshness-first. For a reproducible or rollback run, use
an immutable snapshot URL and pin its source revision:
...
See the [MCP registry trust policy](../../../MCP_REGISTRY_POLICY.md) for the
trust boundary and rollback guidance.
